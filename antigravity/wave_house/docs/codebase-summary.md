...
2. **Data Layer** (`app/src/main/java/com/wavehouse/data/`)
   ...
   - **Remote (Firebase)**: Các lớp implement kết nối trực tiếp Firestore bao gồm `AuthRepositoryImpl`, `OrderRepositoryImpl`, `ProductRepositoryImpl`, `StockRepositoryImpl`, `SupplierRepositoryImpl`, `WarehouseRepositoryImpl`.
...
- Cài đặt triệt để Authentication flow (Login, Register, Forgot Password, Deep Link cho đặt lại mật khẩu) hỗ trợ bởi Firebase Auth.
