...
class CreateStockInUseCase @Inject constructor(
    ...
    suspend operator fun invoke(
        ...
        warehouseId: String,
        quantity: Double,
        /** Giá nhập lô này — null = không cập nhật giá vốn, giữ nguyên MAC hiện tại */
        unitCostPrice: Double? = null,
        supplierId: String? = null,
        note: String? = null
    ): ApiResult<Unit> {
        if (quantity <= 0.0) return ApiResult.Error("Đố lượng nhập phải lớn hơn 0")
        if (unitCostPrice != null && unitCostPrice < 0.0)
            return ApiResult.Error("Giá nhập không được âm")
        return stockRepository.createStockIn(productId, warehouseId, quantity, unitCostPrice, supplierId, note)
    }
}
