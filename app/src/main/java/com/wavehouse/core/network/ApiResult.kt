 ...
 *
 * IMPORTANT: CancellationException luôn được rethrow để tôn trọng coroutine contract.
 */
suspend fun <T> safeApiCall(block: suspend () -> T): ApiResult<T> {
    return try {
        ApiResult.Success(block())
    } catch (e: kotlinx.coroutines.CancellationException) {
        throw e  // CRITICAL: Không được nuốt — vi phạm coroutine contract → memory leak
