...
class ProductRepositoryImpl @Inject constructor(
    ...
    private val priceHistoryRef = database.getReference("product_price_history")
    ...
    override suspend fun createProduct(product: Product): ApiResult<String> = safeApiCall {
        ...
        val pushId = priceHistoryRef.child(id).push().key ?: UUID.randomUUID().toString()
        val initialRecord = mapOf(
            "id" to pushId,
            "costPrice" to p.costPrice,
            "salePrice" to p.salePrice,
            "timestamp" to p.createdAt,
            "updatedBy" to "system",
            "updatedByName" to "Hệ thống",
            "reason" to "Khởi tạo giá sản phẩm"
        )
        
        val updates = hashMapOf<String, Any>(
            "/products/$id" to p.toMap(),
            "/product_price_history/$id/$pushId" to initialRecord
        )
    ...
    override suspend fun updateProductPrice(
        ...
        val pushId = priceHistoryRef.child(productId).push().key ?: UUID.randomUUID().toString()
        ...
        val updates = hashMapOf<String, Any>(
            ...
            "/product_price_history/$productId/$pushId" to record
        )
        database.reference.updateChildren(updates).await()
    }

    override fun getProductPriceHistory(productId: String): Flow<ApiResult<List<com.wavehouse.domain.model.PriceRecord>>> = callbackFlow {
        trySend(ApiResult.Loading)
        val query = priceHistoryRef.child(productId).orderByChild("timestamp")
        val listener = object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                val records = snapshot.children.mapNotNull { it.toPriceRecord() }
                    .sortedByDescending { it.timestamp }
                trySend(ApiResult.Success(records))
            }
            override fun onCancelled(error: DatabaseError) {
                trySend(ApiResult.Error(error.message))
            }
...
private fun DataSnapshot.toProduct(): Product? {
    return try {
        Product(
            ...
            weightPerUnit = child("weightPerUnit").getValue(Double::class.java),
            allowDecimal = child("allowDecimal").getValue(Boolean::class.java) ?: true,
...
private fun DataSnapshot.toPriceRecord(): com.wavehouse.domain.model.PriceRecord? {
    return try {
        com.wavehouse.domain.model.PriceRecord(
            ...
            updatedByName = child("updatedByName").getValue(String::class.java),
...
private fun Product.toMap(): Map<String, Any?> = mapOf(
    ...
    "weightPerUnit" to weightPerUnit,
    "allowDecimal" to allowDecimal,
